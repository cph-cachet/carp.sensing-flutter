...
class PermissionHandler {
  ...
  /// Returns a [Map] containing the [PermissionStatus] for each [PermissionGroup] provided.
  Future<Map<PermissionGroup, PermissionStatus>> requestPermissions(
      List<PermissionGroup> permissions) async {
    final Map<PermissionGroup, PermissionStatus> results = {};
    for (PermissionGroup p in permissions) {
      PermissionStatus s = await _requestSinglePermission(p);
      results[p] = s;
    }
    return results;
  }

  /// Returns the [PermissionStatus] for the request of a single [PermissionGroup]
  Future<PermissionStatus> _requestSinglePermission(PermissionGroup p) async {
    if (p == PermissionGroup.health_data) {
      /// Request authorization (async)
      PermissionStatus s = await Health.requestAuthorization().then(
          (response) =>
              response ? PermissionStatus.granted : PermissionStatus.disabled);
      return s;
    }
    Map<dynamic, dynamic> response =
        await _methodChannel.invokeMethod('requestPermissions', [p.value]);
    return PermissionStatus.values[response[p.value]];
  }
