...
void main() {
  ...
  test('Triggers -> JSON', () async {
    ...
    study_3.addTriggerTask(
        ...
        Task(name: 'Sensing Task #1')..measures = SamplingSchema.common().getMeasureList(types: [SensorSamplingPackage.PEDOMETER, DeviceSamplingPackage.SCREEN]));
    ...
    study_3.addTriggerTask(
        ...
        Task(name: 'Sensing Task #3')..measures = SamplingSchema.common().getMeasureList(types: [AppsSamplingPackage.APP_USAGE, ConnectivitySamplingPackage.BLUETOOTH]));
    ...
    print('$t2');
    study_3.addTriggerTask(t2, Task(name: 'Sensing Task #1')..measures = SamplingSchema.common().getMeasureList(types: [AppsSamplingPackage.APPS, ConnectivitySamplingPackage.CONNECTIVITY]));
    ...
    print('$t3');
    study_3.addTriggerTask(t3, Task(name: 'Sensing Task #1')..measures = SamplingSchema.common().getMeasureList(types: [AppsSamplingPackage.APPS, ConnectivitySamplingPackage.CONNECTIVITY]));
    ...
    print('$t4');
    study_3.addTriggerTask(t4, Task(name: 'Sensing Task #1')..measures = SamplingSchema.common().getMeasureList(types: [AppsSamplingPackage.APPS, ConnectivitySamplingPackage.CONNECTIVITY]));

    // when entering the 'wifi.bardram.net' WIFI network, start sampling bluetooth devices
    study_3.addTriggerTask(SamplingEventTrigger(measureType: MeasureType(NameSpace.CARP, ConnectivitySamplingPackage.WIFI), resumeCondition: WifiDatum()..ssid = 'wifi.bardram.net'),
